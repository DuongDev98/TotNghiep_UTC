...
namespace WebApplication.Controllers
{
    public class NhaCungCapController : Controller
    {
        ...
        public ActionResult CongNo()
        {
            ...
            (
	            CAST(COALESCE((SELECT SUM(COALESCE(TIENTHANHTOAN, CAST(0 AS DECIMAL(18,2))) - COALESCE(TONGCONG, CAST(0 AS DECIMAL(18,2)))) FROM TDONHANG WHERE DNHACUNGCAPID = DNHACUNGCAP.ID), 0) AS DECIMAL(18,2))
	            +
	            CAST(COALESCE((SELECT SUM(COALESCE(CHI, CAST(0 AS DECIMAL(18,2))) - COALESCE(THU, CAST(0 AS DECIMAL(18,2)))) FROM TTHUCHI WHERE DNHACUNGCAPID = DNHACUNGCAP.ID), 0) AS DECIMAL(18,2))
            ) AS CONGNO
            FROM DNHACUNGCAP";
